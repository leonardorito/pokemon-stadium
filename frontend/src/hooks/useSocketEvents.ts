...
export function useSocketEvents(): void {
  useEffect(() => {
    try {
      ...
      useLobbyStore.getState().setError('Failed to connect — check the backend URL');
      return;
    }

    const onJoined = (p: JoinedPayload) => useLobbyStore.getState().applyJoined(p);
    const onLobbyStatus = (p: LobbyStatusPayload) => useLobbyStore.getState().applyLobbyStatus(p);
    const onBattleStart = (p: BattleStartPayload) => useBattleStore.getState().applyBattleStart(p);
    const onBattleSync = (p: BattleSyncPayload) => useBattleStore.getState().applyBattleSync(p);
    const onTurnResult = (p: TurnResultPayload) => useBattleStore.getState().applyTurnResult(p);
    ...
    const onTurnChange = (p: TurnChangePayload) => useBattleStore.getState().applyTurnChange(p);
    const onBattleEnd = (p: BattleEndPayload) => useBattleStore.getState().applyBattleEnd(p);
