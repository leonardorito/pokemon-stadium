...
const log = (state: BattleState, line: string): Pick<BattleState, 'battleLog' | 'lastEvent'> => ({
...
export const useBattleStore = create<BattleState>((set, get) => ({
    ...
    set((state) => {
      ...
      if (team) {
        teams[payload.playerId] = team.map((p) =>
          p.name === payload.pokemonName && !p.defeated ? { ...p, defeated: true, currentHp: 0 } : p,
        );
      }
      return {
        teams,
  ...
  applyPokemonEnter: (payload) => {
    set((state) => log(state, `${formatName(payload.pokemon.name)} enters the battle!`));
  },
